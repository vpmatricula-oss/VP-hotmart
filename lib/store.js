...
export const store = {
  ...
  async addProduct(data) {
    ...
    const product = {
      ...
      livroWooProductId: data.livroWooProductId || '', // id do livro no WooCommerce (cruzamento)
      livroFlowNs: data.livroFlowNs || '',             // flow do ManyChat da mensagem do livro
      livroCoupon: data.livroCoupon || '',             // só conta resgate com esse cupom (opcional)
      livroDesde: data.livroDesde || '',               // só compradores a partir desta data (YYYY-MM-DD)
  ...
  async getLog(id) { return (await loadDB()).logs.find(l => l.id === id) || null; },
  async addLog(entry) {
    const db = await loadDB();
