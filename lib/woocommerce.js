...
// Pedidos que usaram um cupom específico (no produto do livro), a partir de uma data.
// Retorna [{ email, name, orderId, status, date }]. Usado pra detectar vazamento do cupom.
export async function getCouponOrders(bookProductId, coupon, after) {
  const couponLc = String(coupon).toLowerCase().trim();
  const out = [];
  const seen = new Set();
  const base = { product: bookProductId, per_page: 100, status: 'any', _fields: 'id,status,billing,coupon_lines,date_created' };
  if (after) base.after = after;
  const collect = (orders) => {
    for (const o of orders) {
      const codes = (o.coupon_lines || []).map(c => String(c.code).toLowerCase());
      if (!codes.includes(couponLc)) continue;
      ...
      out.push({
        ...
        orderId: o.id, status: o.status, date: o.date_created,
      });
    }
  };
  ...
  if (pages > 1) {
    const rest = await Promise.all(Array.from({ length: pages - 1 }, (_, i) => wooGet('orders', { ...base, page: i + 2 })));
